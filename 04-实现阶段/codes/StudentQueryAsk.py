...
class StudentQueryAsk:
    ...
    def queryAskReq(user,devId):
        ...
        with db.cursor() as cursor:
            sql ="select ask_record.device_id, ask_record.date_ask, ask_record.days, ask_record.date_start, ask_record.is_pass, device.state from (ask_record join device) where %s %s ask_record.device_id = device.id" % (querySqlUserPart, querySqlIdPart)
            cursor.execute(sql)
            data = cursor.fetchall()
        db.close()
        attribute = np.array(['设备id', '申请日期', '申请天数','开始使用日期','审核状态','设备状态'])
        data = np.array(data)
        if(len(data) == 0):
            return attribute
        else:
